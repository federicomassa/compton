...
double stdev(double th){

  //TCanvas* c=new TCanvas("c","C",1000,500);
  //c->Divide(2,1);

  double npoints=1000000;
  TH1F *histo =new TH1F("hist","hist",270,150,4000);
  TH1F *histo2= new TH1F("hist2","hist2",400,10000,4E6);
  TRandom3 rndgen;
  double s_elec;
  double  s_delta;
  double s_firststad;
  ...
  for (int i=0;i<npoints;i++){
    ...
    // double gain;
    s_firststad=s_elec*s_delta;
    //    cout << s_firststad << endl;
    s_firststad2 = s_elec*s_elec*s_delta*s_delta;
...
double Vdev(double th){
  double largh=stdev(th)*echarge*resistence*PMTamp/(3*plastictime);
  return largh;
}

void plotStdev(){
  double gra[5]={40,45,50,55,60};
  double deviation[5];
  for (int u=0;u<5;u++){
    deviation[u]=Vdev(double(gra[u]));
    TGraphErrors* plot=new TGraphErrors(5,gra,deviation, NULL,NULL);

  plot->SetTitle("Signal Standard Deviation (Reconstructed);Theta (gradi);StDev (mV)");
  plot->Draw("APE");
  }
}
